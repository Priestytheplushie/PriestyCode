...
class PriestyCode(tk.Tk):
    ...
    def _configure_styles(self):
        ...
        self.style.configure("Treeview.Heading", font=('Segoe UI', 9, 'bold'), background="#3C3C3C", foreground="white", relief="flat")
    ...
    def _create_menu_bar(self):
        ...
        terminal_menu.add_command(label="Clear Problems", command=lambda: self.error_console.clear())
    ...
    def _create_main_content_area(self):
        ...
        terminal_page.grid_columnconfigure(0, weight=1); terminal_page.grid_rowconfigure(0, weight=1)
        self.terminal_content_frame = tk.Frame(terminal_page); self.terminal_content_frame.grid(row=0, column=0, sticky="nsew")
        self.terminal_tabs_sidebar = tk.Frame(terminal_page, bg="#2B2B2B", width=150); self.terminal_tabs_sidebar.grid(row=0, column=1, sticky="ns"); self.terminal_tabs_sidebar.pack_propagate(False)
        ...
        self.error_console = ConsoleUi(error_page, jump_callback=self._jump_to_error_location)
    ...
    def _create_new_terminal(self):
        ...
        if self.terminal_icon: icon_label.config(image=self.terminal_icon)
        icon_label.pack(side="left", padx=(5, 2))

        name_label = tk.Label(tab_frame, text=f"Terminal {len(self.terminals) + 1}", bg="#2B2B2B", fg="white", font=("Segoe UI", 9), anchor="w")
        name_label.pack(side="left", fill="x", expand=True)
        new_terminal.display_name_widget = name_label

        close_button = tk.Button(tab_frame, text="\u2715", bg="#2B2B2B", fg="#CCCCCC", bd=0, activebackground="#E81123", activeforeground="white", relief="flat", command=lambda t=new_terminal: self._close_terminal(t))
        ...
        self.terminals.append(new_terminal); self.terminal_ui_map[new_terminal] = tab_frame
        self._switch_terminal(new_terminal); self.output_notebook.select(0)
        self._apply_font_size(); return new_terminal

    def _switch_terminal(self, terminal_to_activate: Terminal):
        if self.active_terminal == terminal_to_activate: return
        ...
        if self.active_terminal and self.active_terminal in self.terminal_ui_map:
            ...
            for child in self.terminal_ui_map[self.active_terminal].winfo_children(): child.config(bg="#2B2B2B") # type: ignore
        ...
        if self.active_terminal:
            ...
            if self.active_terminal in self.terminal_ui_map:
                ...
                for child in self.terminal_ui_map[self.active_terminal].winfo_children(): child.config(bg="#3C3C3C") # type: ignore
            self.active_terminal.text.focus_set()

    def _close_terminal(self, terminal_to_close: Terminal):
        if len(self.terminals) <= 1: messagebox.showwarning("Close Terminal", "Cannot close the last terminal.", parent=self); return

        was_active = (self.active_terminal == terminal_to_close)
        
        if terminal_to_close in self.terminal_ui_map: self.terminal_ui_map[terminal_to_close].destroy(); del self.terminal_ui_map[terminal_to_close]
        
        terminal_to_close.destroy(); self.terminals.remove(terminal_to_close)
        
        if was_active: self._switch_terminal(self.terminals[-1] if self.terminals else None) # type: ignore

    def _show_terminal_context_menu(self, event, terminal: Terminal):
        context_menu = tk.Menu(self, tearoff=0, bg="#3C3C3C", fg="white", activebackground="#555555", activeforeground="white")
    ...
    def _rename_terminal(self, terminal: Terminal):
        ...
        if new_name and new_name.strip(): terminal.display_name_widget.config(text=new_name.strip()) # type: ignore

    def _get_run_terminal(self) -> Terminal:
        if not self.active_terminal: return self._create_new_terminal() if not self.terminals else self.terminals[0]
        return self.active_terminal

    def _open_sandbox_if_empty(self):
        if not self.open_files: self._open_new_sandbox_tab()
    ...
    def _check_virtual_env(self):
        ...
        for venv_name in ('.venv', 'venv'): 
            ...
            if os.path.isdir(path):
                ...
                if os.path.exists(potential_path): self.python_executable = os.path.abspath(potential_path); found_venv = True; break 
        
        if not found_venv:
            self.python_executable = sys.executable
            if not self.venv_warning_shown:
                self.venv_warning_shown = True
                if messagebox.askyesno("Virtual Environment Recommended", "No virtual environment was found. It is highly recommended to use one.\n\nWould you like to create one now?"): self._create_virtual_env()
        
        for term in self.terminals: term.set_python_executable(self.python_executable); term.clear(); term.show_prompt()

    def _create_virtual_env(self):
        run_terminal = self._get_run_terminal(); run_terminal.write("Creating virtual environment 'venv'... This may take a moment.\n")
        self.output_notebook.select(0); self.update_idletasks()
        venv_dir = os.path.join(self.workspace_root_dir, "venv")
        if os.path.exists(venv_dir): messagebox.showwarning("Exists", "A 'venv' folder already exists."); return
        def create():
            try:
    ...
    def _change_interpreter(self):
        ...
        if new_path and os.path.exists(new_path):
            self.python_executable = new_path; self.venv_warning_shown = True
            for term in self.terminals: term.set_python_executable(self.python_executable); term.clear(); term.show_prompt()
    ...
    def _install_requirements(self):
        ...
        if not os.path.exists(requirements_path): messagebox.showwarning("Not Found", "No requirements.txt file found in the workspace root.", parent=self); return
        run_terminal = self._get_run_terminal(); self.output_notebook.select(0); self._switch_terminal(run_terminal)
        command = f'"{self.python_executable}" -m pip install -r "{requirements_path}"'
        run_terminal.write(f"Executing: {command}\n", ("info_tag",)); self.update_idletasks()
        run_terminal._handle_shell_command(command)
        
    def _new_file(self, extension=".py"): self._add_new_tab(extension=extension)
    ...
    def _open_folder(self):
        ...
        self.file_explorer.set_project_root(new_path); [term.set_cwd(new_path) for term in self.terminals]
        self._check_virtual_env(); self.title(f"PriestyCode - {os.path.basename(new_path)}")

    def _open_file_from_path(self, file_path):
        if not file_path or file_path == "N/A":
            messagebox.showerror("Error", f"Failed to open file: No such file or directory: '{file_path}'")
            return
            
        _, extension = os.path.splitext(file_path);
        if extension.lower() in self.BINARY_EXTENSIONS: messagebox.showerror("Cannot Open File", f"The file '{os.path.basename(file_path)}' appears to be a binary file."); return
        if os.path.basename(file_path) == "sandbox.py":
            for open_path in self.open_files:
                if os.path.basename(open_path) == "sandbox.py": self._switch_to_tab(self.open_files.index(open_path)); return
            self._open_new_sandbox_tab(); return
    ...
    def _add_new_tab(self, file_path=None, content="", extension=".py"):
        ...
        editor = CodeEditor(editor_frame, error_console=self.error_console, autocomplete_icons={'snippet': self.snippet_icon, 'keyword': self.keyword_icon, 'function': self.function_icon, 'variable': self.variable_icon, 'class': self.function_icon}, autoindent_var=self.autoindent_enabled, tooltips_var=self.tooltips_enabled)
        editor.pack(fill="both", expand=True)

        is_sandbox = (file_path == "sandbox.py"); is_untitled = False
        if file_path and not is_sandbox and not file_path.startswith("Untitled-"):
            try:
                with open(file_path, "r", encoding="utf-8") as f: content = f.read()
            except Exception as e: messagebox.showerror("Error", f"Failed to open file: {e}"); editor_frame.destroy(); return
        elif not file_path or file_path.startswith("Untitled-"):
            is_untitled = True; count = 1; untitled_name = f"Untitled-{count}{extension}"
            while untitled_name in self.open_files: count += 1; untitled_name = f"Untitled-{count}{extension}"
            file_path = untitled_name

        editor.set_file_path(file_path); editor.text_area.insert("1.0", content); editor.text_area.edit_modified(is_sandbox or is_untitled)
        self.after(50, editor._on_content_changed); editor.text_area.bind("<<Change>>", self._schedule_autosave)
        ...
        icon_label = tk.Label(tab, image=icon, bg="#3C3C3C") if icon else tk.Label(tab, bg="#3C3C3C"); icon_label.pack(side="left", padx=(5, 2), pady=2)
    ...
    def _switch_to_tab(self, index: int):
        ...
        self._set_tab_appearance(self.tab_widgets[index], active=True); self.active_editor.text_area.focus_set(); self._update_file_header(self.current_open_file)
    ...
    def _save_file(self, event=None, index=None) -> bool:
        ...
        file_path, editor = self.open_files[idx], cast(CodeEditor, self.editor_widgets[idx].winfo_children()[0])
        if file_path.startswith("Untitled-") or file_path == "sandbox.py": return self._save_file_as(index=idx)
        try:
            with open(file_path, "w", encoding="utf-8") as f: f.write(editor.text_area.get("1.0", "end-1c"))
            editor.text_area.edit_modified(False); return True
    ...
    def _save_file_as(self, event=None, index=None) -> bool:
        idx = self.current_tab_index if index is None else index;
        if not (0 <= idx < len(self.open_files)): return False
        old_path = self.open_files[idx]; initial_file = os.path.basename(old_path) if old_path != "sandbox.py" else "sandbox.py"
        new_path = filedialog.asksaveasfilename(initialdir=self.workspace_root_dir, initialfile=initial_file, defaultextension=".py", filetypes=[("Python", "*.py"), ("All files", "*.*")])
        ...
        try:
            ...
            if not old_path.startswith("Untitled-"): self.handle_file_rename(old_path, new_path)
            else:
                self.open_files[idx], editor = new_path, cast(CodeEditor, self.editor_widgets[idx].winfo_children()[0])
                editor.set_file_path(new_path); editor.text_area.edit_modified(False)
                cast(tk.Label, self.tab_widgets[idx].winfo_children()[1]).config(text=os.path.basename(new_path))
                if idx == self.current_tab_index: self._update_file_header(new_path)
            self.file_explorer.populate_tree(); return True
        except Exception as e: messagebox.showerror("Save Error", f"Failed to save: {e}"); return False

    def _schedule_autosave(self, event=None):
        if self.autosave_timer: self.after_cancel(self.autosave_timer)
        if self.autosave_enabled.get(): self.autosave_timer = self.after(2000, self._perform_autosave)

    def _perform_autosave(self):
        if self.active_editor and self.active_editor.text_area.edit_modified(): self._save_file()
    ...
    def _run_code(self, event=None):
        ...
        self._update_run_stop_button_state(); run_terminal.set_interactive_mode(True); self.output_notebook.select(0); self._switch_terminal(run_terminal)
        file_to_run = self.current_open_file
        if is_sandbox:
            try:
                temp_file = tempfile.NamedTemporaryFile(mode='w+', suffix='.py', delete=False, encoding='utf-8'); temp_file.write(self.active_editor.text_area.get("1.0", "end-1c")); temp_file.close()
                self.temp_run_file = temp_file.name; file_to_run = self.temp_run_file
            except Exception as e: messagebox.showerror("Sandbox Error", f"Could not create temporary file for sandbox: {e}"); self._cleanup_after_run(); return
    ...
    def _start_process_and_threads(self, executable_path, file_path_to_run):
        ...
        try:
            ...
            stdout_thread = threading.Thread(target=self._read_stream_to_queue, args=(self.process.stdout, "stdout_tag"), daemon=True); stderr_thread = threading.Thread(target=self._read_stream_to_queue, args=(self.process.stderr, "stderr_tag"), daemon=True)
    ...
    def _monitor_process(self, stdout_thread: threading.Thread, stderr_thread: threading.Thread):
        if self.process: return_code = self.process.wait(); stdout_thread.join(); stderr_thread.join(); self.output_queue.put((PROCESS_END_SIGNAL, return_code))

    def _execute_in_thread(self, file_to_run_override=None):
        file_path = file_to_run_override or self.current_open_file
        if not file_path: self.output_queue.put((PROCESS_ERROR_SIGNAL, "No file to run.")); return
    ...
    def _stop_code(self, event=None):
        if not self.process or self.process.poll() is not None: self._cleanup_after_run(); return
        run_terminal = self._get_run_terminal()
        if run_terminal:
        ...
        try:
            ...
            if run_terminal:
                run_terminal.write("\n--- Process Killed ---\n", ("stderr_tag",))
        self.process = None; self._cleanup_after_run()
        if run_terminal: run_terminal.show_prompt()

    def _cleanup_after_run(self):
        self.is_running = False; self._update_run_stop_button_state()
        if self.temp_run_file:
            try: os.remove(self.temp_run_file)
            except OSError as e: print(f"Error cleaning up temp file: {e}")
            finally: self.temp_run_file = None

    def _update_run_stop_button_state(self):
        icon, cmd, text = (self.pause_icon, self._stop_code, "Stop") if self.is_running else (self.run_icon, self._run_code, "Run")
    ...
    def _show_about(self): messagebox.showinfo("About PriestyCode", "PriestyCode v1.0.0\nA simple, extensible IDE.\n\nCreated with Python and Tkinter.")
    def _clear_active_output_view(self):
        try:
            if self.output_notebook.tab(self.output_notebook.select(), "text") == "TERMINAL":
                if self.active_terminal and hasattr(self.active_terminal, 'clear'): self.active_terminal.clear();
                if self.active_terminal and not self.active_terminal.interactive_mode: self.active_terminal.show_prompt()
            elif self.output_notebook.tab(self.output_notebook.select(), "text") == "PROBLEMS":
                if hasattr(self.error_console, 'clear'): self.error_console.clear()
        except tk.TclError: pass

    def _open_external_terminal(self):
        try:
            if sys.platform == "win32": subprocess.Popen(f'start cmd /K "cd /d {self.workspace_root_dir}"', shell=True)
            elif sys.platform == "darwin": subprocess.Popen(['open', '-a', 'Terminal', self.workspace_root_dir])
            else:
                try: subprocess.Popen(['gnome-terminal', '--working-directory', self.workspace_root_dir])
                except FileNotFoundError:
                    try: subprocess.Popen(['konsole', '--workdir', self.workspace_root_dir])
                    except FileNotFoundError: subprocess.Popen(['xterm', '-e', f'cd "{self.workspace_root_dir}"; bash'])
        except Exception as e: messagebox.showerror("Error", f"Could not open external terminal: {e}")
            ...
            while not self.output_queue.empty():
                ...
                if char == PROCESS_END_SIGNAL:
                    if isinstance(tag, int):
                        ...
                        if tag != 0 and ("Error" in self.stderr_buffer or "Exception" in self.stderr_buffer):
                            self._handle_error_output(self.stderr_buffer, "Runtime Error", "runtime")
                            ...
                            self.error_console.display_errors([{'title': "Execution Error", 'details': self.stderr_buffer.strip(), 'file_path': 'N/A', 'line': 1, 'col': 1}]); self.output_notebook.select(1)
                    self._cleanup_after_run(); run_terminal.set_interactive_mode(False); self.stderr_buffer, self.stdout_buffer = "", ""
                elif char == PROCESS_ERROR_SIGNAL: self._cleanup_after_run(); run_terminal.set_interactive_mode(False); self.error_console.display_errors([{'title': "Execution Error", 'details': str(tag), 'file_path': 'N/A', 'line': 1, 'col': 1}]); self.output_notebook.select(1)
                else:
                    if run_terminal: run_terminal.write(char, (str(tag),));
    ...
    def _handle_error_output(self, error_text, default_title, highlight_type):
        ...
        if traceback_matches:
            file_path, line_num_str = traceback_matches[-1].groups(); line_num = int(line_num_str)
            ...
            if is_temp_file:
                ...
                for i, open_path in enumerate(self.open_files):
                    if open_path == "sandbox.py": editor_to_highlight = cast(CodeEditor, self.editor_widgets[i].winfo_children()[0]); break
            else:
                norm_error_path = os.path.normcase(os.path.abspath(file_path))
                for i, open_path in enumerate(self.open_files):
                    if open_path and os.path.normcase(os.path.abspath(open_path)) == norm_error_path: editor_to_highlight = cast(CodeEditor, self.editor_widgets[i].winfo_children()[0]); break
            
            if editor_to_highlight:
                method_name = "highlight_handled_exception" if highlight_type == 'handled' else "highlight_runtime_error"
                highlight_method = getattr(editor_to_highlight, method_name, None)
                if highlight_method: self.after(0, highlight_method, line_num, full_error_text)
                        
            error_info = [{'title': error_title, 'details': full_error_text, 'file_path': error_file_path_for_panel, 'line': line_num, 'col': 1}]
            self.error_console.display_errors(error_info, runtime_only=True); self.output_notebook.select(1)
        else: 
             self.error_console.display_errors([{'title': default_title, 'details': full_error_text, 'file_path': 'N/A', 'line': 1, 'col': 1}]); self.output_notebook.select(1)
    ...
    def handle_file_rename(self, old_path, new_path):
        ...
        if os.path.isdir(new_path):
            for i, open_file in enumerate(list(self.open_files)):
                if os.path.normcase(open_file).startswith(old_path_norm + os.sep): self.handle_file_rename(open_file, new_path + open_file[len(old_path):])
            return
        if old_path in self.open_files:
            idx = self.open_files.index(old_path); self.open_files[idx] = new_path
    ...
    def handle_file_delete(self, path: str):
        ...
        for open_file in list(self.open_files):
            if os.path.normcase(open_file).startswith(path_norm): self._close_tab(self.open_files.index(open_file), force_close=True)
    ...
    def _rename_active_file(self):
        if not self.current_open_file or not os.path.exists(self.current_open_file): messagebox.showinfo("Rename File", "Open a saved file to rename it.", parent=self); return
    ...
    def _bind_shortcuts(self):
        self.bind_all("<Control-o>", self._open_file); self.bind_all("<Control-s>", self._save_file); self.bind_all("<Control-S>", self._save_file_as)
        self.bind_all("<Control-f>", self._open_find_replace_dialog); self.bind_all("<F5>", self._run_code); self.bind_all("<Control-F2>", self._stop_code)
        self.bind_all("<Control-z>", self._handle_undo); self.bind_all("<Control-y>", self._handle_redo)
        if sys.platform != "darwin": self.bind_all("<Control-Shift-Z>", self._handle_redo)
        self.bind_all("<F11>", lambda e: self._toggle_fullscreen_event()); self.bind_all("<Escape>", lambda e: self._escape_fullscreen_event())
        self.bind_all("<Control-plus>", self._zoom_in); self.bind_all("<Control-equal>", self._zoom_in); self.bind_all("<Control-minus>", self._zoom_out)
        self.bind_all("<Control-0>", self._reset_zoom); self.bind_all("<Control-MouseWheel>", self._handle_zoom_scroll)
    ...
    def _open_new_window(self): subprocess.Popen([sys.executable, sys.argv[0]])
    def _toggle_fullscreen(self, event=None): self.attributes("-fullscreen", self.fullscreen_var.get())
    def _toggle_fullscreen_event(self, event=None): self.fullscreen_var.set(not self.fullscreen_var.get()); self._toggle_fullscreen()
    def _escape_fullscreen_event(self, event=None):
        if self.fullscreen_var.get(): self.fullscreen_var.set(False); self._toggle_fullscreen()
    
    def _reset_layout(self, event=None):
        self.update_idletasks() 
        try:
            self.main_paned_window.sash_place(0, int(self.main_paned_window.winfo_width() * 0.2), 0)
            self.right_pane.sash_place(0, 0, int(self.right_pane.winfo_height() * 0.7))
        except tk.TclError: self.after(100, self._reset_layout)

    def _zoom_in(self, event=None): self.font_size.set(min(30, self.font_size.get() + 1)); self._apply_font_size(); return "break"
    def _zoom_out(self, event=None): self.font_size.set(max(6, self.font_size.get() - 1)); self._apply_font_size(); return "break"
    def _reset_zoom(self, event=None): self.font_size.set(10); self._apply_font_size(); return "break"
    ...
    def _apply_font_size(self):
        ...
        for editor_frame in self.editor_widgets: cast(CodeEditor, editor_frame.winfo_children()[0]).set_font_size(new_size)
        for term in self.terminals: term.text.config(font=("Consolas", new_size))
...
class FindReplaceDialog(tk.Toplevel):
    def __init__(self, parent, editor: CodeEditor):
        ...
        self.title("Find & Replace"); self.transient(parent); self.geometry("400x150"); self.configure(bg="#3C3C3C"); self.protocol("WM_DELETE_WINDOW", self.close_dialog)
        tk.Label(self, text="Find:", bg="#3C3C3C", fg="white").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.find_entry = tk.Entry(self, bg="#2B2B2B", fg="white", insertbackground="white"); self.find_entry.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        tk.Label(self, text="Replace:", bg="#3C3C3C", fg="white").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.replace_entry = tk.Entry(self, bg="#2B2B2B", fg="white", insertbackground="white"); self.replace_entry.grid(row=1, column=1, padx=5, pady=5, sticky="ew")
    ...
    def find_next(self):
        ...
        if not match_pos: match_pos = self.text_area.search(find_term, "1.0", stopindex=tk.END)
        if match_pos:
            end_pos = f"{match_pos}+{len(find_term)}c"
            self.text_area.tag_remove("sel", "1.0", tk.END); self.text_area.tag_add("sel", match_pos, end_pos)
            self.text_area.mark_set(tk.INSERT, end_pos); self.text_area.see(match_pos); self.text_area.focus_set()
        else: messagebox.showinfo("Find", f"Could not find '{find_term}'", parent=self)
    ...
    def replace(self):
        ...
        if selection:
            start, end = selection
            if self.text_area.get(start, end) == find_term: self.text_area.delete(start, end); self.text_area.insert(start, replace_term)
        self.find_next()
    ...
    def replace_all(self):
        ...
        new_content, replacements = content.replace(find_term, replace_term), content.count(find_term)
        if content != new_content:
            self.text_area.delete("1.0", tk.END); self.text_area.insert("1.0", new_content)
            messagebox.showinfo("Replace All", f"Replaced {replacements} occurrence(s).", parent=self)
        else: messagebox.showinfo("Replace All", "No occurrences found.", parent=self)
    
    def close_dialog(self): self.text_area.tag_remove("sel", "1.0", tk.END); self.destroy()