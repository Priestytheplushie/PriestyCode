...
class FileExplorer(tk.Frame):
    ...
    def populate_tree(self):
        ...
        insert_kwargs = {"parent": "", "index": "end", "iid": self.project_root, "text": root_name, "tags": ('folder',)}
        if self.folder_icon:
        ...
        try:
            ...
            for item in items:
                ...
                if os.path.isdir(full_path):
                    insert_kwargs = {"parent": parent_node, "index": "end", "iid": full_path, "text": item, "tags": ('folder',)}
                    if self.folder_icon:
                        insert_kwargs['image'] = self.folder_icon
    ...
    def _show_context_menu(self, event):
        ...
        if is_file or is_dir:
            self.context_menu.add_command(label="Rename...", command=lambda: self._rename_item(item_id))
            self.context_menu.add_command(label="Move...", command=lambda: self.move_item(item_id))
            self.context_menu.add_command(label="Delete", command=lambda: self._delete_item(item_id))
            self.context_menu.add_separator()
    ...
    def move_item(self, item_id):
        ...
        dest_dir = filedialog.askdirectory(title=f"Move '{os.path.basename(item_id)}' to...", initialdir=initial_dir, parent=self)
        if not dest_dir:
            return
    ...
    def _rename_item(self, item_id):
        ...
        new_name = simpledialog.askstring("Rename", "Enter new name:", initialvalue=current_name, parent=self)
    ...
    def _on_b1_release(self, event):
        ...
        if not drop_target_id or drag_item == drop_target_id: return
    ...
    def _execute_move(self, source_path, dest_path):
        ...
        if os.path.dirname(source_path) == dest_dir: return False
        if os.path.isdir(source_path) and dest_dir.startswith(source_path):
            messagebox.showwarning("Invalid Move", "Cannot move a folder into its own subdirectory.")
            return False
        ...
        try:
            ...
            if os.path.exists(new_full_path):
                if not messagebox.askyesno("Confirm Overwrite", f"'{base_name}' already exists in the destination. Overwrite?"):
                    return False
